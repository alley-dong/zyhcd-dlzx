...
import com.dlzx.system.service.*;
...
public class SysUserController extends BaseController
{
    ...
    @Autowired
    private IRGarageService garageService;
    @Autowired
     ...
     * 根据用户编号获取详细信息
     */
    @PreAuthorize(hasPermi = "system:user:getInfo")
    @GetMapping(value = { "/B01GetInfoL/{userId}" })
    public AjaxResult getInfo(@PathVariable(value = "userId", required = false) @RequestBody Long userId)
    {
        AjaxResult ajax = AjaxResult.success();
...
//        user.setCreateBy(SecurityUtils.getUsername());
        if (!VerifyCodeUtil.checkPasswordRule(user.getPassword())){
            return AjaxResult.error("密码长度必须8位及以上，包含大小写字母，数字，特殊字符。");
        }
        user.setPassword(SecurityUtils.encryptPassword(user.getPassword()));
//        String garageID =garageService.selectIdByName(user.getGarageName());
//        user.setGarageId(garageID);
...
//        user.setUpdateBy(SecurityUtils.getUsername());
     ...
     * 删除用户
     */
    @PreAuthorize(hasPermi = "system:user:delete")
    @Log(title = "用户管理", businessType = BusinessType.DELETE)
    @GetMapping("/B01Delete/{userId}")
     ...
     * 状态修改
     */
    @PreAuthorize(hasPermi = "system:user:changeStatus")
    @Log(title = "用户管理", businessType = BusinessType.UPDATE)
    @PostMapping("/B01ChangeStatus")
