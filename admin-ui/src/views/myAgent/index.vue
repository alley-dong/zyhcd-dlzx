    ...
    <div class="app-container">
      ...
      <div class="box">
        ...
        <div style="text-align: left; font-size: 0.25rem; padding-left: 0.6rem">
          ...
          <div class="text">申请时间：{{ item.createTime.substring(0,item.createTime.lastIndexOf(':')) }}</div>
      </div>
          <!-- <div class="text">
            结束时间：{{ item.vEndTime }}
            <span
...
import { P01ExamineBacklogListt } from "@/api/myAgent";
import { Toast } from "vant";
import { userGetInfo as userGetInfo_name, logout } from "@/api/login";
export default {
  components: {
    Header,
  },
  ...
  mounted() {
    console.log(userGetInfo_name);
      userGetInfo_name().then((res) => {
  ...
  methods: {
    getList() {
      P01ExamineBacklogListt(this.queryParams).then((res) => {
