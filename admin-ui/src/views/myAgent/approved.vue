    ...
    <div class="app-container leave">
      ...
      <van-cell-group>
        ...
        <div v-for="item in tableData" :key="item.id">
          ...
          <van-cell
            title="工龄工资："
            :value="item.wageForSeniority"
            title-class="left-title"
          />
          <van-cell
            title="绩效工资："
            :value="item.meritSalary"
            title-class="left-title"
          />
            <van-cell
            title="加班补助："
...
import { C06Detail } from "@/api/approvalRecord";
import { sysExamineTaskList } from "@/api/com";
import { salaryRecordsC08Review } from "@/api/myAgent";
...
export default {
 ...
 handleAddClick(status) {
  ...
  mounted() {
    C06Detail(this.$route.params.id).then((res) => {
      console.log(res);
      this.tableData = res.data.LongRecord;
    });
