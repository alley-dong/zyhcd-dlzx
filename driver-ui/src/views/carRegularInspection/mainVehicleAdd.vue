...
import { minioChooseBucketUploadList,queryExamineConfigProcessList } from "@/api/api";
import {
  inspectI02Add,
  inspectI04List,
  inspectI07Add,
  ...
  methods: {
    getIndex(arr, key) {
      ...
      while (low <= high) {
        ...
        if (key == arr[mid].dictValue) {
          return mid;
        } else if (key > arr[mid].dictValue) {
          low = mid + 1;
        } else {
          high = mid - 1;
        }
