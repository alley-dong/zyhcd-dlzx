<template>
  <div class="app-container">
    ...
    <el-form label-width="150px" class="textColor">
      <el-form-item style="display:inline-block" label="车辆年检费合计(元):">{{
        totalCost.totalVehiceInspectCost
      }}</el-form-item>
      <el-form-item style="display:inline-block" label="营运年检费合计(元):">{{
        totalCost.totalOperateInspectCost
      }}</el-form-item>
      <el-form-item style="display:inline-block" label="复检费合计(元):">{{
        totalCost.totalReviewCost
      }}</el-form-item>
      <el-form-item style="display:inline-block" label="检车费合计(元):">{{
    ...
    <el-table :data="cardList" border>
      ...
      <el-table-column label="检车日期" align="center" prop="inspectDate" />
      <el-table-column
        label="车辆年检费用"
        align="center"
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      M08List(this.queryParams).then(response => {
    ...
    /** 搜索按钮操作 */
    handleQuery() {
      let {startTime,finshTime} = this.queryParams
      if (startTime && finshTime&&(startTime > finshTime)) {
        this.$message({
          showClose: true,
    ...
    handleExport() {
      this.download(
        "/pfserver-car/car/inspection/M08Export",
        {
          ...this.queryParams
        },
        `system_inspection.xlsx`
      );
    },
    /** 提交按钮 */
    submitForm() {
