<template>
  <div class="app-container">
    ...
    <el-form label-width="100px" class="textColor">
      <el-form-item style="display: inline-block" label="费用合计:">{{
      ...
      <div style="width: 640px; height: 200px">
        ...
        >
          ...
          <div class="el-upload__tip" slot="tip">
            ...
            <a href="/newLicense.xlsx" download="新购车辆临时号牌费.xlsx">下载模板</a>
          </div>
        </el-upload>
      </div>
...
import {
  FuelRechargeList,
  M13PurchasesCostImport,
} from "@/api/card/recordOfCharging";
import {
  M13PurchasesCostList,
  M06FuelCostPullDown,
  // M13PurchasesCostImport,
...
export default {
  ...
  data() {
    return {
      ...
      queryParams: {
        ...
        oilQuantity: "",
        amount: "",
        beginTime: this.parseTime(new Date(), "{y}-{m}") + "-01",
        unitPrice: "",
        taxRate: "",
        endTime:this.parseTime(new Date(), "{y}-{m}-{d}"),
      },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      M13PurchasesCostList(this.queryParams).then((response) => {
    ...
    handleExport() {
      this.download(
        "/pfserver-car/purchasesCost/M13PurchasesCostExport",
        {
          ...this.queryParams,
        },
        `新购车辆临时号牌费.xlsx`
