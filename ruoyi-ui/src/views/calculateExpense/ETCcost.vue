  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item style="float: right">
        ...
        <el-button
          ...
          size="small "
          @click="handleExport"
          v-hasPermi="['car:etcCost:export']"
          >导出</el-button
        >
      </el-form-item>
    </el-form>
    <el-form label-width="120px" class="textColor">
      <el-form-item style="display: inline-block" label="含税(元):">{{
        totalCost.taxAmountAll
      }}</el-form-item>
    ...
    <el-table :data="cardList" border>
      ...
      <el-table-column
        ...
        prop="licensePlate"
      />
      <el-table-column label="金额（含税）" align="center" prop="taxAmount" />
      <el-table-column label="金额（元不含税）" align="center" prop="amount" />
      <el-table-column label="税额" align="center" prop="tax" />
      <!-- <el-table-column label="报销时间" align="center" prop="reimburseTime" /> -->
      <el-table-column label="报销时间" align="center">
        <template slot-scope="scope">
          {{ scope.row.reimburseTime | time }}
        </template>
      </el-table-column>
    </el-table>
    <pagination
      ...
      <div style="width: 640px; height: 200px">
        ...
        >
          ...
          <div class="el-upload__tip" slot="tip">
            <a href="/etcFee.xls" download="ETC费导入模板.xls">下载模板</a>
          </div>
        </el-upload>
      </div>
...
// import {FuelRechargeList,I11FuelRechargeImport} from '@/api/card/recordOfCharging'
import { M07List, M07ImportData } from "@/api/calculateExpense/ETCcost";
export default {
  name: "ETCcost",
  data() {
    return {
      ...
      queryParams: {
        ...
        // bTime: undefined,
        // eTime: undefined,
         bTime: this.parseTime(new Date(),'{y}-{m}')+'-01',
        eTime: this.parseTime(new Date(),'{y}-{m}-{d}'),
        taxAmount: undefined,
        amount: undefined,
        tax: undefined,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      M07List(this.queryParams).then((response) => {
        // console.log(response)
        this.cardList = response.rows[0].list;
        let { bTime, eTime } = this.queryParams;
        if (bTime && eTime) {
    ...
    handleExport() {
      this.download(
        "/pfserver-car/car/etcCost/M07Export",
        {
          ...this.queryParams,
        },
        `system_etcCost.xlsx`
