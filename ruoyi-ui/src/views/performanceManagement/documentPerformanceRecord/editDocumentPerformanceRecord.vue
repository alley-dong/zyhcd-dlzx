...
import { G10Edit } from "@/api/performanceManagement/editorLongDistancePerformanceRecords";
  ...
  methods: {
    ...
    submitForm(elForm) {
      this.$refs["elForm"].validate((valid) => {
        if (valid) {
          G10Edit(this.formData).then((res) => {});
          this.resetback();
        }
      });
      // this.$router.push({ path: "/editorLongDistancePerformance/" + this.$route.params.id })
    },
    resetback() {
      this.$router.back();
    },
