...
import { VehicleSave, vehicleInfo, vhicleSelectBox } from "@/api/car/vehicle";
export default {
  components: {},
  props: [],
  data() {
  ...
  methods: {
    ...
    vehicleChange(val) {
      ...
      this.vhicleSelectBoxData(val, true);
    },
    ...
    vehicleInfoDetail() {
      vehicleInfo(this.$route.params.id).then(res => {
        ...
        this.fileUrl = this.formData.fileUrl ? this.formData.fileUrl : [];
        let arr = [];
        ...
        if (this.formData.vehicleType) {
          this.vhicleSelectBoxData(this.formData.vehicleType, true);
        }
      });
    },
    submitForm() {
      ...
      this.formData.fileUrl = arr;
      // debugger
      VehicleSave(this.formData).then(res => {
        console.log(res);
        this.$router.go(-1);
      });
    },
    ...
    handleRemove(file, fileList) {
      for (const [index, item] of this.fileUrl.entries()) {
        if (item.name == file.name && item.src == file.src) {
          console.log(index);
    ...
    UploadFileGroups(param) {
      ...
      UploadFiles(fd)
        .then(response => {
          // _this.fileUrl.length--
          _this.fileUrl.push({
            url: response.msg,
            name: response.msg.substring(response.msg.lastIndexOf("/") + 1)
          });
