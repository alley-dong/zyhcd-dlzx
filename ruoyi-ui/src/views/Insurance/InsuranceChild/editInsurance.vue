      ...
      <el-form ref="elForm" :model="formData" size="medium" label-width="100px">
        ...
        <el-col :span="12">
          <el-form-item label="终保日期">
            <div>
              {{ parseTime(formData.updateTime, "{y}-{m}-{d} {h}:{i}:{s}") }}
            </div>
          </el-form-item>
        </el-col> -->
        <el-col :span="24">
...
<script>
import { insuranceVehicleE02GetInfo, insuranceVehicleListE03Check } from "@/api/system/vehicle";
...
export default {
  ...
  methods: {
    ...
    editClick() {
      insuranceVehicleListE03Check(this.formData1).then().then(this.backClick());
    },
    backClick() {
      this.$router.go(-1);
    },
