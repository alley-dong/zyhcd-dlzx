      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            plain
            size="small"
          v-hasPermi="['pfservercar:vehicle:editsave']"
            >审批</el-button
          >
          <el-button
            type="primary"
          ...
          v-hasPermi="['pfservercar:vehicle:list']"
            >查看</el-button
          >
          <!-- <el-button
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      insuranceVehicleList06(this.queryParams).then((response) => {
    ...
    handleApproval(id) {
      ...
      this.$router.push({ path: "/goodsEditInsurance/" + id });
    },
    /** 搜索按钮操作 */
    handleQuery() {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      this.$router.push({ path: "/insuranceDetail/" + row.id });
    },
    /** 导出按钮操作 */
    handleExport() {
      this.download(
