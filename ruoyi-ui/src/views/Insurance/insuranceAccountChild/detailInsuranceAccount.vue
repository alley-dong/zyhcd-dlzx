<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="日期" prop="insuranceHandleTime">
        <el-date-picker
          clearable
          size="small"
          style="width: 200px"
          v-model="queryParams.insuranceHandleTime"
          type="date"
      ...
      <el-form-item style="float:right"> 
        <el-button
          ...
          >搜索</el-button
        >
        <el-button type="primary" size="mini" @click="$router.go(-1)" style="width:73px;height:29px"
          > 返回</el-button
        >
      </el-form-item>
    </el-form>
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <!-- {h}:{i}:{s} -->
          <span>{{
            parseTime(scope.row.insuranceEndDate, "{y}-{m}-{d} ")
          }}</span>
        </template>
      </el-table-column>
      <el-table-column
...
<script>
import { googdsE15List } from "@/api/system/vehicle";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      googdsE15List(this.queryParams).then((response) => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      this.$router.push({ path: "/seeInsuranceAccount/" + row.id,query:{vehicleId:row.vehicleId} });
    },
    /** 导出按钮操作 */
    handleExport() {
      this.download(
