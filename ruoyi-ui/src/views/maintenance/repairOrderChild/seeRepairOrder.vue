  ...
  <div class="app-container">
    <div style="overflow: auto">
      <div style="float: right">
        ...
        <!--size="small"  -->
        <el-button v-if="$route.query.status==1" type="primary" @click="removeClick()">撤销</el-button>
      </div>
    </div>
    <div class="border">
        ...
        >
          ...
          <div v-for="item in formData.repairItemDate" :key="item.id">
            ...
            <el-form-item label="维修完成时间">
              <div>{{ formData.completeTime }}</div>
            </el-form-item>
          </el-col>
            <el-col :span="24">
            <el-form-item label="维修时长">
              <div>{{ formData.repairDuration }}</div>
            </el-form-item>
          </el-col>
          <!-- <el-col :span="24">
    ...
    <div v-if="formData.fileList" style="text-align:left">
      <img
        ...
        style="width:200px;height:200px;margin-right:20px"
      />
      <!-- <img alt="" srcset="" style="width:200px;height:200px;margin-right:20px">
      <img alt="" srcset="" style="width:200px;height:200px;margin-right:20px">
      <img alt="" srcset="" style="width:200px;height:200px;margin-right:20px">
      <img alt="" srcset="" style="width:200px;height:200px;margin-right:20px">
      <img alt="" srcset="" style="width:200px;height:200px;margin-right:20px">
    ...
    >
      ...
      <el-table-column label="审批时间" align="center">
        <template slot-scope="scope">
          <span v-if="scope.row.reviewTime">{{ scope.row.reviewTime | data   }}</span>
        </template>
      </el-table-column>
...
// 维修保养查看
    repairInfo(this.$route.params.id).then((res) => {
      ...
      console.log("res.data.fileList",res.data.fileList)
      let temp = []
      if(res.data.fileList){
        res.data.fileList.forEach(r=>{
          let obj = {
            url:r
          }
          temp.push(obj)
      ...
      this.formData.fileList = temp
      console.log("formData.fileList",this.formData.fileList)
      
    });
// 查看审核中的维修明细
    repairDetailAllList(this.$route.params.id).then((res) => {
        console.log('查看审核中的维修明细',res);
        this.tableData1 = res.data.detailedList;
