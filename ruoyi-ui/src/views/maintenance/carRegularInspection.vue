<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="定检类型" align="center" prop="type">
        <template slot-scope="scope">
          {{ scope.row.type == 1 ? "主车定检" : "挂车定检" }}
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      vehicleInspectH19List(this.queryParams).then((response) => {
    ...
    handleUpdate(row) {
      ...
      // this.reset();
      const id = row.id || this.ids;
      this.$router.push({ path: "/editCarRegularInspection/" + row.id,query:{vehicleId:row.vehicleId} });
    },
    /** 提交按钮 */
    submitForm() {
