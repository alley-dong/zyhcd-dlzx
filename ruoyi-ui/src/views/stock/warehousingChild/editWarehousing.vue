<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="入库数量" prop="inventoryNumber">
        <el-input
...
export default {
  ...
  mounted() {
    ...
    goodsInventoryGetInfo(this.$route.params.id ).then(
      ...
      this.codeOptation(res.data.goodsTypeId);
      }
    );
    // 物品类别下拉请求
  ...
  methods: {
    ...
    goodsTypeChange(val) {
      ...
      console.log(val);
      this.codeOptation(this.formData.goodsTypeId);
    },
    codeOptation(id) {
    ...
    editClick() {
      console.log("新增");
      goodsInventoryK05EditSave(this.formData).then((res) => {
        this.backClick();
      });
    },
  },
