<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="出库单号" align="center" prop="code" />
      <el-table-column label="出库时间" align="center" prop="inventoryTime" />
      ...
      <el-table-column label="出库数量" align="center" prop="inventoryNumber" />
      <el-table-column label="车牌号" align="center" prop="licensePlate" />
      <!-- TODO:金额 -->
      <el-table-column label="金额（元）" align="center" prop="total" />
      <el-table-column label="出库类别" align="center" prop="inventoryType">
      ...
      >
        <template slot-scope="scope">
          <el-button
            v-if="scope.row.inventoryType !== 1"
            size="small"
            type="primary"
          ...
          <el-button
            size="small"
          v-if="scope.row.inventoryType==3"
...
import {
  goodsInventoryK06List,
  goodsInventoryK06Delete,
  goodsTypeK01List,
...
export default {
  ...
  methods: {
       handleImportant() {
      this.download(
        "/pfserver-parts/parts/goodsInventory/goodsInventoryOutListExport",
        {
          ...this.queryParams,
        },
        `出库.xlsx`
    ...
    getList(bool) {
      this.loading = true;
      goodsInventoryK06List(this.queryParams).then((response) => {
    ...
    // 查看
    seeClick(row) {
      this.$router.push("/seeDelivery/" + row.id);
    },
    // 取消按钮
    cancel() {
      this.open = false;
    ...
    handleUpdate(row) {
      ...
      this.$router.push("/editDelivery/" + id);
    },

    /** 删除按钮操作 */
    handleRemove(row) {
      const id = row.id || this.ids;
      // goodsInventoryK06Delete(id).then((res) => {
      //   this.getList();
      // });
      let that = this;
      that
        ...
        .then(function () {
          goodsInventoryK06Delete(id).then((response) => {
            that.getList();
            that.getListOption();
            that.$message({
              message: "删除成功",
