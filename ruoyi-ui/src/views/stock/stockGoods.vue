      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="单价" prop="unitPrice">
          <el-input
            ...
            clearable
          />
          <div style="width:10%;float:right;text-align:center">元</div>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getList(bool) {
      this.loading = true;
      goodsK02List(this.queryParams).then((response) => {
        // if (bool) {
    ...
    handleUpdate(row) {
      ...
      goodsK02EditSave1(id).then((response) => {
        ...
        let flg = true;
        for (const item of this.repairListOption) {
          if (this.form.goodsTypeId === item.id) {
            flg = false
          }
        }
        if(flg){
    ...
    handleRemove(row) {
      ...
      that
        ...
        .then(function () {
          goodsK02Delete(id).then((response) => {
            that.getList();
            that.getListOption();
            that.$message({
              message: "删除成功",
