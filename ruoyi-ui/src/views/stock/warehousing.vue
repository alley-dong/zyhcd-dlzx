<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="入库时间" align="center" prop="inventoryTime" />
      ...
      <el-table-column label="入库数量" align="center" prop="inventoryNumber" />
      <!-- TODO:金额 -->
      <el-table-column label="金额（元）" align="center" prop="total" />
      <el-table-column label="入库类别" align="center" prop="inventoryType">
...
报废
  ...
  methods: {
    handleImportant() {
      this.download(
        "/pfserver-parts/parts/goodsInventory/goodsInventoryInListExport",
        {
          ...this.queryParams,
        },
        `入库.xlsx`
    ...
    getList(bool) {
      this.loading = true;
      goodsK03List(this.queryParams).then((response) => {
        // if (bool) {
        //   this.repairListOption = response.rows;
        // }
    ...
    handleUpdate(row) {
      ...
      this.$router.push("/editWarehousing/" + id);
    ...
    handleRemove(row) {
      ...
      that
        ...
        .then(function () {
          goodsInventoryK03Delete(id).then((response) => {
            that.getList();
            that.$message({
              message: "删除成功",
              type: "success",
