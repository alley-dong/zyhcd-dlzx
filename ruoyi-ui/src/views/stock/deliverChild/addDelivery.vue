...
<script>
import { goodsTypeK01List,
goodsK02List,
goodsInventoryK07Add 
} from "@/api/stock/index";
// import {goodsInventoryK07Add} from '@/api/stock/child/index'
export default {
  data() {
    return {
      formData: {
  ...
  methods: {
    ...
    goodsTypeChange(val) {
      ...
      console.log(val);
      this.formData.goodsCode = undefined
      this.formData.goodsName = undefined
      this.formData.specification = undefined
        this.formData.unit = undefined
      this.formData.unitPrice = undefined
      this.formData.supplier = undefined
    ...
    editClick() {
      console.log("新增");
      goodsInventoryK07Add(this.formData).then(res=>{
        this.backClick()
      })
    },
  },
